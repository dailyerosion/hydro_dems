...
# def mosaicDEMsAndPitfill(demListVal, maskRastBase, huc12, log, sgdb, terrains, procDir, fElevFile, interpDict, named_cell_size,
...
#     try:
#         arcpy.env.cellSize = int(demListVal)#[0]
#         maskRastOut = opj(sgdb, maskRastBase + demListVal)
...
#             fElevFile = updateResolution(fElevFile, named_cell_size, demListVal, pattern22, log)
...
#             #         log.debug('---Mosaicing DEMs for ' + demListVal + ')
#             #         mosDEM = arcpy.MosaicToNewRaster_management(mosaicList, procDir, 'mos_' + demListVal + 'm_' + huc12 + '.tif', number_of_bands = '1', pixel_type = '32_BIT_FLOAT', mosaic_method = 'MINIMUM')
...
#             #         log.debug('---Resampling DEMs for ' + demListVal)
#             #         mosDEM = arcpy.Resample_management(mosaicList[0], os.path.join(procDir, 'mos_' + demListVal + 'm_' + huc12 + '.tif'), cell_size = arcpy.env.cellSize, resampling_type = arcpy.env.resamplingMethod)
#             #         maskedDEM = Con(Plus(maskRastOut, 2), mosDEM)
#             #         maskedDEMintPre = Int(maskedDEM*100)
...
#             maskedDEMint.save('msk_dem_' + demListVal + 'm_' + huc12 + '.tif')
...
#             log.debug('Saved DEM for ' + demListVal)# + ' at ' + str(time.clock()))
