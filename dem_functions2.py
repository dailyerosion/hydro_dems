...
##        "soilsDir" : opj(os.path.dirname(acpfBase), 'dep_WEPP_SOL'+ str(int(ACPFyear))),
        ...
        if int(ACPFyear) <= 2015:
            geeTillageMap = opj(otherBaseNoVersion, 'Iowa_RC.gdb\\huc' + huc8 + '_ACPF2017')
        elif int(ACPFyear) <= 2025:
            geeTillageMap = opj(otherBaseNoVersion, 'GEE_residue_cover', 'residue_cover_' + ACPFyear + '.tif')
        if int(ACPFyear) <= 2017:
            mnTillageMap = opj(otherBaseNoVersion, 'Minnesota\\2017_combo_agonly_recount_combo_residue_times2.img')
        elif int(ACPFyear) <= 2020:
            mnTillageMap = opj(otherBaseNoVersion, 'Minnesota\\minnesota_2020_boax_rowcrop_noforage_residue_times2_rc200_new_combo2019_2020_model.img')
        elif int(ACPFyear) == 2021:
            mnTillageMap = opj(otherBaseNoVersion, 'Minnesota\\minnesota_2021_boax_rowcrop_noforage_residue_times2_rc200_new_combo2019_2020_model.img')
        elif int(ACPFyear) == 2022:
            mnTillageMap = opj(otherBaseNoVersion, 'Minnesota\\minnesota_s2_boa_wgs15x_residue_times2_rowcrops_noforage_final_w_flood_mask_out.tif')
        ...
        "mnMedianTillage" : mnTillageMap,
        "geeMedianTillage" : geeTillageMap,
...
##        "geeTillageTable" : opj(locationsDictNoVersion["huc8_fields"], 'huc' + huc8 + '_gee_rc' + ACPFyear),
        "mnTillageTable" : opj(ACPFDir, 'huc' + huc12 + '_mn_rc' + ACPFyear),
        "geeTillageTable" : opj(ACPFDir, 'huc' + huc12 + '_gee_rc' + ACPFyear),
